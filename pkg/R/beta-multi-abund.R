beta.multi.abund<-function (x, index.family = "bray") 
{
    ...
    maxbibj <- sum(x$pair.max.not.shared)
    minbibj <- sum(x$pair.min.not.shared)
    switch(index.family, bray = {
        beta.bray.bal <- minbibj/(minbibj + x$multiple.shared)
        beta.bray.gra <- (x$multiple.shared/(minbibj + x$multiple.shared)) * ((maxbibj - minbibj)/((2 * 
            x$multiple.shared) + maxbibj + minbibj))
        beta.bray <- (minbibj + maxbibj)/(minbibj + maxbibj + 
            (2 * x$multiple.shared))
        multi <- list(beta.BRAY.BAL = beta.bray.bal, beta.BRAY.GRA = beta.bray.gra, 
            beta.BRAY = beta.bray)
    }, ruzicka = {
        beta.ruz.bal <- (2 * minbibj)/((2 * minbibj) + x$multiple.shared)
        beta.ruz.gra <- (x$multiple.shared/((2 * minbibj) + x$multiple.shared)) * ((maxbibj - 
            minbibj)/((x$multiple.shared) + maxbibj + minbibj))
